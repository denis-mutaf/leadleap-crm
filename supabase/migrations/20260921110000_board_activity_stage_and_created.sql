-- Последняя активность на карточке воронки включает смену этапа и создание.
--
-- Доска считала активностью только заметки и звонки, а таблица сделок — ещё и
-- смену этапа. Из-за этого одна и та же сделка на доске писала «Активность пока
-- не зафиксирована», а в таблице — «Этап изменён · 19.09, 12:47». Колонка
-- «Сообщения» выглядела мёртвой, хотя данные были.
--
-- Источников теперь четыре. Заметка и звонок всегда в приоритете — у них есть
-- текст; смена этапа и создание сделки подставляются, только если человек
-- ещё ничего не сделал. Пустой строки активности на карточке больше нет.

create index if not exists stage_transitions_deal_changed_idx
  on public.stage_transitions (deal_id, changed_at desc);
  ...
  from page r
    ...
    left join lateral (
      ...
      from (
        ...
        from notes n where n.deal_id = r.id and n.deleted_at is null
        union all
