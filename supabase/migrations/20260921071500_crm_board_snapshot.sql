-- Срез доски одним запросом.
--
-- Экран воронки делал около сорока обращений к Postgres четырьмя волнами:
-- отдельный count и отдельная выборка на каждую из тринадцати колонок, затем
-- две волны догрузок связанных сущностей. Из Молдовы это 130-360 мс за обращение,
-- отсюда «кликнул и пять секунд ничего не происходит».
--
-- Здесь то же самое считается в базе за один вызов: колонки, счётчики,
-- страница карточек с контактом, ответственным, источником, метками, проектами,
-- ближайшей задачей и последней активностью. Фильтры и сортировка выполняются
-- тоже здесь, а не в JS над уже выкачанной страницей.
--
-- security invoker: RLS продолжает решать, что видит менеджер, а что руководитель.
