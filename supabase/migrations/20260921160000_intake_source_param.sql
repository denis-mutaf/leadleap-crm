-- Лид-форма Facebook попадает в ту же обработку, что и форма сайта: телефон,
-- имя, поиск дубля, сделка на первом этапе. Отличается только источником,
-- поэтому он стал параметром, а не константой.
--
-- Старая восьмиаргументная функция снимается: если оставить обе, вызов
-- с восемью аргументами станет неоднозначным для Postgres.
drop function if exists public.process_web_form_event(uuid, text, text, text, jsonb, text, jsonb, jsonb);

create or replace function public.process_web_form_event(p_event_id uuid, p_phone text, p_name text, p_comment text, p_utm jsonb, p_meta_campaign_id text, p_unknown jsonb, p_unknown_labels jsonb, p_source_code text default 'web_form')
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
revoke all on function public.process_web_form_event(uuid, text, text, text, jsonb, text, jsonb, jsonb, text) from public, anon, authenticated;
grant execute on function public.process_web_form_event(uuid, text, text, text, jsonb, text, jsonb, jsonb, text) to service_role;
