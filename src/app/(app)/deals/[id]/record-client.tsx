...
import { CalendarDays, Check, CheckSquare, Clock3, FileText, Hash, List, MessageCircle, Phone, Plus, Trash2, Type, User, X } from "lucide-react";
...
export function DealRecordClient({ data }: { data: DealRecordData }) {
  ...
  return <main className={`${styles.main} record-main`}>
    ...
    <div className={styles.body}><aside className={styles.left}>
      ...
      <div className={styles.quickActions}><form onSubmit={createNote}><input value={note} onChange={(event) => setNote(event.target.value)} placeholder="Написать примечание…" aria-label="Примечание" /><button className="btn" disabled={saving || !note.trim()}><Plus size={14} /> Примечание</button></form><form onSubmit={createTask}><input ref={taskInputRef} value={taskTitle} onChange={(event) => setTaskTitle(event.target.value)} placeholder="Новая задача" aria-label="Название задачи" /><DateField type="datetime-local" value={taskDue} onChange={setTaskDue} aria-label="Дата и время задачи" placeholder="Срок" /><button className="btn btn-primary" disabled={saving || !taskTitle.trim() || !taskDue}><Plus size={14} /> Задача</button></form></div>
    ...
    {completionTask && <div className={`${styles.modalBackdrop} motion-veil`}><form className={`${styles.modal} motion-dialog`} onSubmit={completeTask}><div className={styles.modalHead}><h2>Результат задачи</h2><button type="button" onClick={() => setCompletionTask(null)} aria-label="Закрыть"><X size={16} /></button></div><p>{completionTask.title}</p><textarea autoFocus required value={completionResult} onChange={(event) => setCompletionResult(event.target.value)} placeholder="Что сделано" /><div className={styles.modalActions}><button type="button" onClick={() => setCompletionTask(null)}>Отмена</button><button type="submit" disabled={saving || !completionResult.trim()}>{saving ? "Сохраняем…" : "Выполнить задачу"}</button></div></form></div>}
