...
function CustomValueField({ definition, value, dealId, fallback = null, label }: { definition: DealFieldDefinition; value: DealFieldValue | undefined; dealId: string; fallback?: string | null; label?: string }) {
  const router = useRouter(); const [current, setCurrent] = useState(valueToString(value?.value) ?? fallback); const [editing, setEditing] = useState(false); const [draft, setDraft] = useState(current ?? ""); const [saving, setSaving] = useState(false); const options = normalizeOptions(definition.options); const display = current && definition.field_type === "select" ? options.find((option) => option.value === current)?.label ?? current : current;
  ...
  return <div className={`field-row ${saving ? styles.fieldSaving : ""}`}><span>{label ?? definition.label}</span>{editing ? definition.field_type === "select" ? <select className="field-input" autoFocus value={draft} onChange={(event) => commit(event.target.value)} onBlur={() => setEditing(false)} onKeyDown={(event) => event.key === "Escape" && setEditing(false)}><option value="">— не выбрано —</option>{options.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select> : <input className="field-input" autoFocus type={definition.field_type === "number" ? "number" : definition.field_type === "date" ? "date" : "text"} value={draft} onChange={(event) => setDraft(event.target.value)} onBlur={(event) => commit(event.target.value)} onKeyDown={(event) => { if (event.key === "Escape") setEditing(false); if (event.key === "Enter") commit(draft); }} /> : <button type="button" className={`field-value ${current ? "" : "is-empty"}`} onClick={() => { setDraft(current ?? ""); setEditing(true); }} title="Нажмите, чтобы изменить">{display ?? "—"}</button>}</div>;
}
...
function StageField({ data }: { data: DealRecordData }) {
  ...
  return <div className="field-row"><span>Этап</span>{editing ? <select className="field-input" autoFocus disabled={busy} defaultValue={data.stage?.id ?? data.deal.stage_id} onChange={(event) => save(event.target.value)} onBlur={() => setEditing(false)}>{data.allStages.map((stage) => <option key={stage.id} value={stage.id}>{stage.name}</option>)}</select> : <button type="button" className="field-value" onClick={() => setEditing(true)}>{data.stage?.name ?? "—"}</button>}</div>;
}
...
export function DealRecordClient({ data }: { data: DealRecordData }) {
  ...
  const customDefinitions = data.fieldDefs.filter((definition) => !isAttributionField(definition.key, definition.label) && !canonicalFieldKey(definition.key, definition.label));
  ...
  const namedField = (semanticKey: string, label: string, column: string, value: string | null, type?: "text" | "number" | "select", options?: { value: string; label: string }[]) => {
    ...
    return definition
      ? <CustomValueField definition={definition} value={customValues.get(definition.id)} dealId={data.deal.id} fallback={value} label={label} />
      : type === "select"
        ? <InlineField label={label} table="deals" id={data.deal.id} column={column} type="select" value={value} options={options ?? []} />
  ...
  return <main className={`${styles.main} record-main`}>
    ...
    <div className={styles.body}><aside className={styles.left}>
      ...
      <section className={styles.section}><h3>Квалификация</h3>{namedField("construction_stage", "Этап строительства", "construction_stage", data.deal.construction_stage ?? null)}<InlineField label="Ежемесячный платёж" table="deals" id={data.deal.id} column="monthly_payment_text" value={(data.deal.monthly_payment_text ?? fmtMoney(data.deal.monthly_payment, data.deal.budget_currency)) ?? null} /><InlineField label="Первый взнос" table="deals" id={data.deal.id} column="down_payment_text" value={(data.deal.down_payment_text ?? fmtMoney(data.deal.down_payment, data.deal.budget_currency)) ?? null} />{namedField("payment", "Способ оплаты", "payment", data.deal.payment ?? null, "select", [{ value: "cash", label: "Наличные" }, { value: "mortgage", label: "Ипотека" }, { value: "installment", label: "Рассрочка" }])}<InlineField label="Срок покупки" table="deals" id={data.deal.id} column="purchase_timing_text" value={(data.deal.purchase_timing_text ?? data.deal.horizon) ?? null} /><InlineField label="В стране" table="deals" id={data.deal.id} column="residency_detail" value={(data.deal.residency_detail ?? data.deal.residency) ?? null} /><InlineField label="Цель покупки" table="deals" id={data.deal.id} column="purpose" type="select" value={data.deal.purpose} options={[{ value: "living", label: "Для жизни" }, { value: "investment", label: "Инвестиция" }]} />{namedField("rooms", "Комнатность", "rooms", data.deal.rooms === null ? null : String(data.deal.rooms), "number")}{namedField("area_m2", "Площадь, м²", "desired_area_text", data.deal.desired_area_text ?? null)}{namedField("floor", "Этаж", "desired_floor_text", data.deal.desired_floor_text ?? null)}<InlineField label="Пожелания" table="deals" id={data.deal.id} column="wishes" type="textarea" value={data.deal.wishes ?? null} /></section>
